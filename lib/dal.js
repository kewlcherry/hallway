...
exports.query = function(sql, binds, cbDone) {
  ...
  var curQuery = {sql:sql, binds:_.clone(binds), start:startTime};
  curQueries.push(curQuery);
  pool.acquire(function(error, db) {
  curQuery.acquired = Date.now();
    if (error) return cbDone(new Error(error));

    return db.query(sql, binds, function(error, rows, res) {
      curQueries.splice(curQueries.indexOf(curQuery), 1);
      pool.release(db);
...
exports.multiquery = function(sqls, cbDone) {
  ...
  var startTime = Date.now();
  var curQuery = {sql:"multiquery "+sqls.length, start:startTime};
  curQueries.push(curQuery);
  pool.acquire(function(error, db) {
    if (error) return cbDone(new Error(error));
    ...
    return db.multiquery(sqls, function(ret) {
      ...
      // this is sort of temporary as multiquery should be doing them in batch
      curQueries.splice(curQueries.indexOf(curQuery), 1);
      curQuery = {sql:sql, start:Date.now()};
      curQueries.push(curQuery);
    });
  });
};
...
exports.getCurQueries = function() {
  curQueries.forEach(function(query) {
    query.total = (Date.now() - query.start) / 1000;
  });
  return curQueries;
}
