...
var kstore = require('key').store(lconfig.taskman.store || 'mem');
var redis;
      ...
      tasksNow(function(err, tasks) {
        ...
        if (!pi || typeof pi.auth !== "object") {
          return async.forEach(Object.keys(tasks), function(taskid, cbLoop) {
            ...
            kstore.del(taskid, function() { cbLoop(); });
          }, function() {
...
function runTask(pi, task, fnCallback) {
  ...
  function cbDone(err, response) {
    ...
    var callback = function(err) {
      ...
      if (sandboxed) {
        configUpdate = {};
        if (cfg) configUpdate[task.synclet] = cfg;
  ...
  // don't actually run a task that has been attempted and never finished too many times
  if (task.attempts >= TASK_ATTEMPTS)
  {
    logger.warn("task attempted too many times", task.attempts, task.idr);
    return cbDone("too many failed attempts to run");
  }
...
exports.freshenUp = function(synclets, service, pid, force, callback) {
  ...
  async.forEach(synclets, function(synclet, cbLoop) {
    ...
    locksmith.isLocked(pid, function(locked){ // this will clear stale locks too
      ...
      runWorker(pid, callback, tasks, force);
    });
  });
};
